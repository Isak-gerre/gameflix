...
export async function getStaticProps() {
	...
	console.log(id);
	const games = await getGamesData(id, "ordering=released");
	return {
		props: {
			games,
			id: id
...
export default function Newlyadded({ games, id }) {
			...
			<section className={Styles.main}>
				...
				<div className={Styles.genre_container}>
					<div className={Styles.genre_cards}>
						...
						<div onClick={async () => {
							setPage(page + 1)
							const games = await getGamesData(id, `ordering=released&page=${page}`);
